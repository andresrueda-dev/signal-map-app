...
if firebase_active: 
    st.sidebar.success("📡 Conectado a Firebase Cloud")
else: 
    st.sidebar.warning("🛡️ Ejecución Estable en Modo Local")
...
def cargar_sorteo_real(game):
    ...
    if os.path.exists(ruta):
        try:
            df = pd.read_csv(ruta)
            # Relleno de nulos para sorteos antiguos de Tris
            df[cols] = df[cols].fillna(0).astype(int)
            return df, cols
        except:
            pass
            
    # Respaldo automático si el archivo está cargándose en GitHub
...
# Menú de Navegación
menu = st.sidebar.radio("Navegación", ["📖 Diario de Señales", "📊 Timeline de Hoy", "🏠 Dashboard Global", "🎯 Sorteo Número Sugerido", "🪞 Motor Espejo"])
...
if menu == "📖 Diario de Señales":
    ...
    if st.button("🚀 Guardar Señal de Hoy"):
        if numeros:
            ...
            st.success("✅ ¡Señal guardada en la sesión!")
            if firebase_active:
...
# 2. TIMELINE
    ...
    if len(st.session_state.local_signals) > 0: 
        ...
        st.info("Aún no hay señales registradas.")
...
# 3. DASHBOARD GLOBAL
    ...
    for game in GAME_CONFIG.keys():
        ...
        with cols_layout[contador_col]:
            st.markdown(f'<div class="dashboard-card"><h3>🎲 {game}</h3><p><b>Volumen Real Indexado:</b> {len(df):,} sorteos | <b>Dominante Histórico:</b> {dominante}</p><p><b>Máxima Persistencia:</b> {persistencia} apariciones</p></div>', unsafe_allow_html=True)
    ...
    for game in GAME_CONFIG.keys():
        ...
        for idx, valores_fila in enumerate(muestras):
            ...
            puntos_mapeados.append({"Eje X": x, "Eje Y": y, "Capa": f"Historial {game}"})
...
# 4. SORTEO SUGERIDO (RESONANCIA EUCLIDIANA)
